...
internal sealed class BookingServiceStub : BaseServiceStub<Booking>, IBookingService
{
    ...
    public Task TryFinalizeLandlordFundsReleaseAsync(Guid bookingId, string source) => Task.CompletedTask;
    ...
    public Task<(IReadOnlyList<OccupiedRoomAlternativeOptionDto> Items, int TotalCount)> FindAlternativeApartmentsAsync(Guid bookingId, int page = 1, int pageSize = 5, double? radiusKilometers = null) => Task.FromResult((Items: (IReadOnlyList<OccupiedRoomAlternativeOptionDto>)Array.Empty<OccupiedRoomAlternativeOptionDto>(), TotalCount: 0));
    public Task<(IReadOnlyList<OccupiedRoomAlternativeAssessmentDto> Items, int TotalCount)> EvaluateAlternativeApartmentsAsync(Guid bookingId, int page = 1, int pageSize = 5, double? radiusKilometers = null) => Task.FromResult((Items: (IReadOnlyList<OccupiedRoomAlternativeAssessmentDto>)Array.Empty<OccupiedRoomAlternativeAssessmentDto>(), TotalCount: 0));
...
internal sealed class LandlordWalletServiceStub : ILandlordWalletService
{
    ...
    public Task ReleasePendingToAvailableAsync(Guid landlordId, decimal amount) => Task.CompletedTask;
